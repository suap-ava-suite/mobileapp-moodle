...
import { MM, App } from './namespace';
    ...
    function buildSplashBrand(logoSrc: string): string {
        return (
            '<div class="ava-splash__brand">' +
                SPLASH_GAUGE_SVG +
    ...
    function bindRetry(button: HTMLElement | null): void {
        if (!button) {
            return;
        }
    ...
    function showErrorFallback(message: string, canRetry: boolean): void {
        if (!App.content) {
            return;
        }
